...
export function VisualizadorCotizacionModular({
  ...
  const handleCompartirWhatsApp = () => {
    ...
    const urlCotizacion = window.location.href;
    const nombreCliente =
      cotizacion.clienteNombrePersonalizado ||
      cotizacion.prospectoNombre ||
      "";
    const saludo = nombreCliente ? `¡Hola! Te comparto la propuesta de ${nombreCliente}` : "¡Hola! Te comparto la propuesta";
    const textoMensaje = `${saludo} con SAUCEDA (${datosModulares.titulo}):\n\n${urlCotizacion}\n\nPuedes revisar la visualización 3D fotorrealista, el plano estructural y personalizar los acabados en tiempo real.`;
    const urlWa = `https://api.whatsapp.com/send?text=${encodeURIComponent(textoMensaje)}`;
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 font-sans text-slate-800">
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-xl shadow-slate-200/40 print:border-none print:shadow-none">
        ...
        <div className="border-b border-slate-100 bg-slate-50/60 p-6 sm:p-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="shrink-0 text-left sm:text-right space-y-2">
              <div>
                ...
                <div className="mt-0.5 text-[11px] text-slate-500">{fechaFormateada}</div>
                <div className="text-[11px] font-semibold text-slate-400">Vigencia: 14 días naturales</div>
              </div>

              {/* Acciones de Compartir Enlace */}
              <div className="flex items-center gap-1.5 sm:justify-end pt-1 print:hidden">
                <button
                  type="button"
                  onClick={handleCompartirWhatsApp}
                  className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 px-3 py-1.5 text-xs font-bold text-white shadow-xs transition active:scale-95 cursor-pointer"
                  title="Compartir cotización por WhatsApp con alguien más"
                >
                ...
                <button
                  ...
                  className="rounded-lg border border-slate-300 hover:border-slate-400 bg-white hover:bg-slate-50 px-2.5 py-1.5 text-xs font-semibold text-slate-700 shadow-2xs transition cursor-pointer"
          ...
          {/* Ficha de Prospecto / Datos del Cliente */}
          {(cotizacion.prospectoNombre || cotizacion.clienteNombrePersonalizado || cotizacion.prospectoTelefono || cotizacion.prospectoDireccion) && (
            <div className="mt-5 flex flex-wrap items-center gap-y-2 gap-x-4 rounded-xl border border-slate-200/80 bg-white px-4 py-3 text-xs text-slate-600 shadow-2xs">
              <div className="flex items-center gap-1.5 font-semibold text-slate-900">
                <span className="text-base">👤</span>
                <span className="text-slate-400 uppercase text-[10px] tracking-wider font-bold">Cliente:</span>
                <span className="text-[#1E3A2F] font-bold text-sm">
                  {cotizacion.clienteNombrePersonalizado || cotizacion.prospectoNombre || cotizacion.contactoNombre || "Cliente"}
                </span>
              ...
              {cotizacion.prospectoTelefono && (
                <div className="flex items-center gap-1.5 border-l border-slate-200 pl-3">
                  ...
                  <a href={`tel:${cotizacion.prospectoTelefono}`} className="font-mono hover:text-[#1E3A2F] font-medium">
                    {cotizacion.prospectoTelefono}
                  </a>
                </div>
              )}
              ...
              {cotizacion.prospectoCorreo && (
                <div className="flex items-center gap-1.5 border-l border-slate-200 pl-3">
                  ...
                  <a href={`mailto:${cotizacion.prospectoCorreo}`} className="hover:text-[#1E3A2F]">
                    {cotizacion.prospectoCorreo}
                  </a>
                </div>
              )}
              ...
              {cotizacion.prospectoDireccion && (
                <div className="flex items-center gap-1.5 border-l border-slate-200 pl-3">
                  <span className="text-slate-400">📍</span>
                  <span className="max-w-xs truncate text-slate-700" title={cotizacion.prospectoDireccion}>
                    {cotizacion.prospectoDireccion}
                  </span>
                </div>
              )}
          ...
          {/* GALERÍA VISUAL: RENDER FOTORREALISTA Y DISEÑO ESTRUCTURAL */}
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Foto 1: Render Fotorrealista ("Cómo se vería real") */}
            <div
              onClick={() =>
                setFotoModal({
                  url: datosModulares.fotos?.renderUrl || "/images/render-pergola-real.jpg",
                  titulo: datosModulares.fotos?.renderTitulo || "Visualización 3D / Render Arquitectónico",
                  descripcion:
                    datosModulares.fotos?.renderDescripcion ||
                    "Simulación estética fotorrealista de la propuesta con acabados seleccionados, integración a los muros y protección perimetral.",
                })
              }
              className="group relative cursor-pointer overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-xs transition-all hover:shadow-md hover:border-emerald-600/50"
            >
              <div className="relative aspect-[16/9] w-full overflow-hidden bg-slate-100">
                <img
                  src={datosModulares.fotos?.renderUrl || "/images/render-pergola-real.jpg"}
                  alt="Render Arquitectónico Fotorrealista"
                  className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
                  loading="lazy"
                />
                <div className="absolute top-3 left-3">
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-[#1E3A2F]/90 backdrop-blur-xs px-2.5 py-1 text-[11px] font-bold text-white shadow-xs">
                    <span>✨</span>
                    <span>Cómo se vería real (Render 3D)</span>
                  </span>
                </div>
                <div className="absolute bottom-3 right-3 rounded-lg bg-black/60 backdrop-blur-xs px-2.5 py-1 text-[10px] font-semibold text-white opacity-0 transition-opacity group-hover:opacity-100">
                  🔍 Clic para ampliar
                </div>
              </div>
              <div className="p-3.5">
                <h4 className="font-titular text-sm font-bold text-[#1E3A2F]">
                  {datosModulares.fotos?.renderTitulo || "Visualización Arquitectónica Fotorrealista"}
                </h4>
                <p className="mt-1 text-xs text-slate-500 line-clamp-2 leading-relaxed">
                  {datosModulares.fotos?.renderDescripcion ||
                    "Integración estética de estructura metálica terminada con cubierta aislante y plafón de madera en armonía con el entorno."}
                </p>
              </div>
            </div>

            {/* Foto 2: Diseño Estructural del Desarrollo */}
            <div
              onClick={() =>
                setFotoModal({
                  url: datosModulares.fotos?.estructuralUrl || "/images/plano-estructural-desarrollo.jpg",
                  titulo: datosModulares.fotos?.estructuralTitulo || "Ingeniería Estructural & Desarrollo de Taller",
                  descripcion:
                    datosModulares.fotos?.estructuralDescripcion ||
                    "Despiece isométrico estructural: columnas PTR, vigas maestras, largueros de cubierta y placas de anclaje químico.",
                })
              }
              className="group relative cursor-pointer overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-xs transition-all hover:shadow-md hover:border-emerald-600/50"
            >
              <div className="relative aspect-[16/9] w-full overflow-hidden bg-slate-900">
                <img
                  src={datosModulares.fotos?.estructuralUrl || "/images/plano-estructural-desarrollo.jpg"}
                  alt="Diseño Estructural del Desarrollo"
                  className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
                  loading="lazy"
                />
                <div className="absolute top-3 left-3">
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-slate-900/90 backdrop-blur-xs px-2.5 py-1 text-[11px] font-bold text-sky-300 shadow-xs border border-sky-400/30">
                    <span>📐</span>
                    <span>Diseño Estructural del Desarrollo</span>
                  </span>
                </div>
                <div className="absolute bottom-3 right-3 rounded-lg bg-black/60 backdrop-blur-xs px-2.5 py-1 text-[10px] font-semibold text-white opacity-0 transition-opacity group-hover:opacity-100">
                  🔍 Clic para ampliar
              ...
              <div className="p-3.5">
                <h4 className="font-titular text-sm font-bold text-[#1E3A2F]">
                  {datosModulares.fotos?.estructuralTitulo || "Diseño Estructural & Plano de Taller"}
                </h4>
                <p className="mt-1 text-xs text-slate-500 line-clamp-2 leading-relaxed">
                  {datosModulares.fotos?.estructuralDescripcion ||
                    "Esquema isométrico de taller: perfiles estructurales PTR, contraventeos calculados y anclajes químicos certificados."}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-8 p-6 sm:p-8">
          ...
          {/* Banner para compartir por WhatsApp */}
          <div className="rounded-2xl border border-emerald-200/80 bg-gradient-to-br from-emerald-50/70 to-emerald-100/30 p-5 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-700 shadow-xs print:hidden">
            <div className="flex items-start gap-3">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-emerald-600 text-white text-lg shadow-xs">
                💬
              </div>
              <div className="space-y-0.5">
                <div className="font-bold text-slate-900 text-sm">
                  ¿Quieres consultar este proyecto con tu socio, arquitecto o pareja?
                </div>
                <p className="text-xs text-slate-600 leading-relaxed">
                  Comparte el enlace directo para que exploren los renders 3D, el plano estructural y el costo en tiempo real.
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 shrink-0 w-full sm:w-auto">
              <button
                type="button"
                onClick={handleCompartirWhatsApp}
                className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 px-4 py-2.5 text-xs font-bold text-white shadow-xs transition active:scale-95 cursor-pointer"
              >
              ...
              <button
                type="button"
                onClick={handleCopiarEnlace}
                className="rounded-xl border border-slate-300 hover:border-slate-400 bg-white hover:bg-slate-50 px-3.5 py-2.5 text-xs font-semibold text-slate-700 shadow-2xs transition cursor-pointer"
              >
                {copiado ? "✓ ¡Copiado!" : "🔗 Copiar link"}
              </button>
            </div>
            ...
            <div className="rounded-2xl border-2 border-emerald-600/30 bg-white p-6 space-y-4 shadow-sm print:hidden">
              ...
              <form onSubmit={handleAutorizar} className="space-y-4 max-w-lg">
                ...
                <button
                  type="submit"
                  disabled={cargando}
                  className="w-full rounded-xl bg-[#1E3A2F] hover:bg-[#1E3A2F]/90 py-3 text-sm font-bold text-white shadow-md transition disabled:opacity-50 cursor-pointer"
                >
      ...
      {/* Modal Lightbox de Foto / Render en Alta Resolución */}
      {fotoModal && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-3 sm:p-6 backdrop-blur-xs animate-in fade-in duration-200"
          onClick={() => setFotoModal(null)}
        >
          <div
            className="relative max-h-[90vh] max-w-4xl w-full overflow-hidden rounded-2xl bg-slate-950 border border-slate-800 shadow-2xl flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between border-b border-slate-800 px-4 py-3 text-white">
              <h3 className="font-titular text-sm sm:text-base font-bold text-emerald-400">
                {fotoModal.titulo}
              </h3>
              <button
                type="button"
                onClick={() => setFotoModal(null)}
                className="rounded-lg bg-slate-800 hover:bg-slate-700 px-2.5 py-1 text-xs font-bold text-slate-300 transition"
              >
                ✕ Cerrar
              </button>
            </div>
            ...
            {fotoModal.descripcion && (
              <div className="border-t border-slate-800 bg-slate-900/90 px-4 py-3 text-xs text-slate-300">
                {fotoModal.descripcion}
              </div>
            )}
          </div>
