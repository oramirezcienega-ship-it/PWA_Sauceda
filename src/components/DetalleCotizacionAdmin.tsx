...
export function DetalleCotizacionAdmin({
  ...
  // --- State para Envío Multicanal / Distribución ---
  const [mostrarModalDistribucion, setMostrarModalDistribucion] = useState<boolean>(false);

  const handleEnviarCorreoWrapper = async (datos: {
    correoDestino: string;
    asunto: string;
    notasAdicionales: string;
  }) => {
    return await enviarCotizacionPorCorreo({
  ...
  return (
    <div className="space-y-6">
                  ...
                  {cotizacion.estatus === "aprobada" ? (
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-carbon/5 space-y-3">
                      ...
                      <div className="flex gap-2 pt-2 flex-wrap items-center">
                        {/* Botón Principal: Previsualizar y Enviar Multicanal */}
                        <button
                          type="button"
                          onClick={() => setMostrarModalDistribucion(true)}
                          className="inline-flex items-center gap-2 rounded-lg bg-verde-profundo text-white px-4 py-2.5 text-xs font-bold hover:bg-sauce transition shadow-md cursor-pointer"
                        >
                          <span className="text-sm">📤</span>
                          <span>Previsualizar y Enviar (WhatsApp / Correo)</span>
                        </button>
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-2 rounded-lg bg-slate-100 border border-carbon/15 text-carbon/80 px-3.5 py-2 text-xs font-semibold hover:bg-slate-200 transition shadow-xs"
                        >
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-2 rounded-lg bg-[#25D366] text-white px-3.5 py-2 text-xs font-semibold hover:bg-[#128C7E] transition shadow-xs disabled:opacity-50"
                        >
                          {/* SVG Whatsapp */}
                          <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                            <path d="M12.012 2c-5.506 0-9.989 4.478-9.99 9.984a9.96 9.96 0 001.37 5.054L2 22l5.13-1.346a9.945 9.945 0 004.88 1.28c5.505 0 9.988-4.478 9.989-9.984C22.01 6.477 17.528 2 12.012 2zm6.36 14.195c-.277.78-1.6 1.436-2.23 1.5-1.12.1-3.21-.6-5.71-3.1-2.07-2.07-3.07-4.14-3.07-5.13 0-1.12.77-1.74 1.1-2.04.28-.26.54-.3.72-.3.17 0 .34 0 .5.01.16 0 .38-.06.58.42.2.49.7 1.7.77 1.83.07.13.1.28.01.46-.09.18-.18.3-.32.46-.14.16-.3.36-.43.48-.15.14-.3.29-.13.58.18.29.8 1.3 1.7 2.1.86.76 1.8 1.14 2.1 1.28.3.14.47.12.65-.08.18-.2.78-.9.98-1.2.2-.3.4-.26.68-.16.27.1 1.73.81 2.03.96.3.15.5.22.58.36.08.14.08.82-.2 1.6z"/>
                          </svg>
                          <span>{enviandoAPI ? "Enviando..." : "Enviar por CRM (API Meta)"}</span>
                        </button>
                      </div>
                    </div>
                  )}
      ...
      {/* Modal de Previsualización y Envío Multicanal (WhatsApp / Correo) */}
      <ModalPrevisualizarCotizacion
        abierto={mostrarModalDistribucion}
