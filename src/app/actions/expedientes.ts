...
export async function listarExpedientes(): Promise<Expediente[]> {
  ...
  let query = sb
    .from("expedientes")
    .select("*, prospectos(origen, campaign_name, adset_name, ad_name, correo, direccion), asesor:asesor_id(nombre), operador:operador_id(nombre)");
...
export async function obtenerExpedientesSeguimiento(): Promise<ExpedienteSeguimiento[]> {
  ...
  const tipoNegocioLabels: Record<string, string> = {
    ...
    "construccion_mantenimiento_cisternas": "Mantenimiento Cisternas",
    "mantenimiento_cisternas": "Mantenimiento Cisternas",
    "cisternas": "Mantenimiento Cisternas",
