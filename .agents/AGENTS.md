...
* **Políticas de Git y Ramas**:
  ...
  * **AUTO PUSH A AgenteIDE OBLIGATORIO:** Siempre que se complete y valide una tarea, corrección o funcionalidad, realizar automáticamente el `git add`, `git commit` y `git push origin AgenteIDE` sin necesidad de que el usuario lo solicite.
  * Todos los cambios de código, confirmaciones y empujes deben realizarse estrictamente en la rama `AgenteIDE` (o ramas de características que deriven de ella).
  * El usuario se encargará de realizar el merge correspondiente y detonar el despliegue final.
